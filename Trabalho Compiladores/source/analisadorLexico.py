...
# def verificarToken(afd, token, tabela_de_simbolos, num_linha):
    ...
    for simbolo in token:
        if simbolo in afd[estado]:  # Verifica se o simbolo está presente na posição atual
            estado = afd[estado][simbolo][0]  # Atualiza a posição para a próxima encontrada
        else:
            tabela_de_simbolos.append((num_linha, token, "x"))
            return  # Se o simbolo não estiver presente, o token não foi reconhecido
    #aeiouε
    if '*' in afd[estado]:   # Após verificar todos os simbolos, verifica se a posição atual é um estado final
        tabela_de_simbolos.append((num_linha, token, estado))
        return # Se a posição atual for final reconheceu o token
    else:
        if 'Îµ' in afd[estado]:
            estado = afd[estado]["Îµ"][0] # caso termine de ler o token e o estado não seja final, ele tenta fazer uma eplison transição
            if '*' in afd[estado]: 
                tabela_de_simbolos.append((num_linha, token, estado))
                return # Se a posição atual for final reconheceu o token
        tabela_de_simbolos.append((num_linha, token, "x"))
        return # Se a posição atual não for final o token não foi reconhecido
